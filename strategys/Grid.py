...
class GridStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        perc_level = [
            x for x in np.arange(1 + 0.02 * 5, 1 - 0.02 * 5 - 0.02 / 2, -0.02)
        ]
        # 价格区间
        # print(self.mid)
    ...
    def next(self):
        ...
        if self.last_price_index == None:
            ...
            for i in range(len(self.price_levels)):
                ...
                if self.data.close[0] > self.price_levels[i]:
                    ...
                    self.order_target_percent(target=i /
        ...
        # 调仓
            ...
            if signal:
                ...
                self.order_target_percent(target=self.last_price_index /
                                          (len(self.price_levels) - 1))
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                self.log('执行买入, 价格: %.2f, 成本: %.2f, 手续费 %.2f' %
                         (order.executed.price, order.executed.value,
                          order.executed.comm))
                self.buyprice = order.executed.price
                self.comm += order.executed.comm
            else:
                self.log('执行卖出, 价格: %.2f, 成本: %.2f, 手续费 %.2f' %
                         (order.executed.price, order.executed.value,
                          order.executed.comm))
                self.comm += order.executed.comm
